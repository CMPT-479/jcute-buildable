...
public class ParseExpr {
     ...
     * @param isCond
     */
    public static void instrument(Body b, Value v, Value left, Chain units, Stmt s, SymbolTable st, boolean isConcurrent, int lineNo, boolean isCond) {
        if (v instanceof Local || v instanceof ArrayRef || v instanceof InstanceFieldRef || v instanceof StaticFieldRef) {
            AddCallWithAddress.instrument(v, units, s, "loadAddress", true, st, lineNo, false);
        }
        if (v instanceof Local || v instanceof ArrayRef || v instanceof InstanceFieldRef || v instanceof StaticFieldRef || v instanceof Constant) {
            AddValue.instrument(b, v, units, s, "loadValue", true, lineNo, isCond);
        } else if (v instanceof AddExpr || v instanceof SubExpr || v instanceof MulExpr || v instanceof CmpExpr || v instanceof CmpgExpr || v instanceof CmplExpr) {
            if (!(((BinopExpr) v).getOp1() instanceof Constant && ((BinopExpr) v).getOp2() instanceof Constant)) {
                ...
                Value sym = StringConstant.v(((BinopExpr) v).getSymbol());
                units.insertBefore(Jimple.v().newInvokeStmt(Jimple.v().newStaticInvokeExpr(mr, sym, IntConstant.v(lineNo))), s);
            }
        } else if (v instanceof EqExpr || v instanceof LeExpr || v instanceof LtExpr || v instanceof GeExpr || v instanceof GtExpr || v instanceof NeExpr) {
            if (!(((BinopExpr) v).getOp1() instanceof Constant && ((BinopExpr) v).getOp2() instanceof Constant)) {
            ...
            mr = Scene.v().getMethod("<cute.concolic.Call: void funBegin(java.lang.String,int)>").makeRef();
            units.insertBefore(Jimple.v().newInvokeStmt(Jimple.v().newStaticInvokeExpr(mr, StringConstant.v("NewArray"), IntConstant.v(lineNo))), s);
            ...
            mr = Scene.v().getMethod("<cute.concolic.Call: void funBegin(java.lang.String,int)>").makeRef();
            units.insertBefore(Jimple.v().newInvokeStmt(Jimple.v().newStaticInvokeExpr(mr, StringConstant.v("ArrayLength"), IntConstant.v(lineNo))), s);
            if (left != null) {
                AddCallWithAddress.instrument(left, units, s, "storeReturn", false, st, lineNo, false);
            }
